...
export type ShapeConfig =
  | { shape_type: 'rect'; width: number; height: number; corner_radius?: number; fill?: string; stroke?: StrokeConfig }
  | { shape_type: 'ellipse'; width: number; height: number; fill?: string; stroke?: StrokeConfig }
  | { shape_type: 'line'; x1: number; y1: number; x2: number; y2: number; stroke: StrokeConfig }
  | { shape_type: 'polygon'; points: [number, number][]; fill?: string; stroke?: StrokeConfig }
...
export type GradientConfig =
  | { gradient_type: 'linear'; colors: GradientStop[]; start: [number, number]; end: [number, number] }
  | { gradient_type: 'radial'; colors: GradientStop[]; center: [number, number]; radius: number }

/** Transition between sequence layers. */
export type TransitionConfig =
  | { type: 'crossfade'; duration: number }
  | { type: 'wipe'; duration: number; direction: 'left' | 'right' | 'up' | 'down' }
  | { type: 'slide'; duration: number; direction: 'left' | 'right' | 'up' | 'down' }
...
export type EffectConfig =
  ...
  | { type: 'drop_shadow'; color: string; offset_x: number; offset_y: number; blur: number; opacity?: number }
  | { type: 'glow'; color: string; radius: number; intensity?: number }
  | { type: 'brightness_contrast'; brightness?: number; contrast?: number }
  | { type: 'hue_saturation'; hue?: number; saturation?: number; lightness?: number }
  | { type: 'invert' }
 ...
 */
function serializeTransform(t?: TransformConfig): Record<string, unknown> {
  const result: Record<string, unknown> = {}

  if (t?.position !== undefined) {
    result.position = isKeyframeArray(t.position)
      ? serializeKeyframes(t.position as Keyframe<[number, number]>[])
      : t.position
  }

  if (t?.scale !== undefined) {
    result.scale = isKeyframeArray(t.scale)
      ? serializeKeyframes(t.scale as Keyframe<[number, number]>[])
      : t.scale
  }

  if (t?.rotation !== undefined) {
    result.rotation = isKeyframeArray(t.rotation)
      ? serializeKeyframes(t.rotation as Keyframe<number>[])
      : t.rotation
  }

  if (t?.opacity !== undefined) {
    result.opacity = isKeyframeArray(t.opacity)
      ? serializeKeyframes(t.opacity as Keyframe<number>[])
      : t.opacity
  }
 ...
 */
function isKeyframeArray(v: unknown): boolean {
...
function serializeKeyframes<T>(kfs: Keyframe<T>[]): Record<string, unknown>[] {
  return kfs.map(kf => {
    const entry: Record<string, unknown> = { t: kf.frame, v: kf.value }
...
interface LayerBase {
  ...
  motion_blur?: boolean
  adjustment?: boolean
  time_remap?: { speed?: number; offset?: number; reverse?: boolean }
  path_animation?: { points: [number, number][]; auto_orient?: boolean }
  track_matte?: { source: string; mode?: string }
  trim_paths?: { start?: number; end?: number; offset?: number }
  [key: string]: unknown
}
...
export class Scene {
  ...
  private isSequence = false
  private sequenceTransition?: TransitionConfig
  ...
  addShape(id: string, opts: {
    ...
    parent?: string
    motionBlur?: boolean
    pathAnimation?: { points: [number, number][]; autoOrient?: boolean }
    trimPaths?: { start?: number; end?: number; offset?: number }
  }): this {
    this.layers.push({
      id,
      type: 'shape',
 ...
 * ```
 */
export function keyframes<T>(kfs: Keyframe<T>[]): Keyframe<T>[] {
...
export function spring(config?: {
  ...
  return [
    { frame: 0, value: from, easing: { type: 'spring' as const, stiffness, damping, mass } },
...
export const Easing = {
  ...
  /** Ease in (slow start). */
  easeIn: 'ease_in' as const,
  /** Ease out (slow end). */
  easeOut: 'ease_out' as const,
  /** Ease in-out (slow start and end). */
  easeInOut: 'ease_in_out' as const,
  /**
 ...
 * ```
 */
export function rect(width: number, height: number, opts?: {
  cornerRadius?: number
  fill?: string
  stroke?: StrokeConfig
}): ShapeConfig {
 ...
 * ```
 */
export function ellipse(width: number, height: number, opts?: {
  fill?: string
  stroke?: StrokeConfig
}): ShapeConfig {
  return {
 ...
 * ```
 */
export function line(x1: number, y1: number, x2: number, y2: number, stroke: StrokeConfig): ShapeConfig {
 ...
 * ```
 */
export function polygon(points: [number, number][], opts?: {
  fill?: string
  stroke?: StrokeConfig
}): ShapeConfig {
  return {
 ...
 * ```
 */
export function star(outerRadius: number, innerRadius: number, numPoints: number, opts?: {
  fill?: string
  stroke?: StrokeConfig
}): ShapeConfig {
...
export const Effects = {
   ...
   */
  blur: (radius: number): EffectConfig => ({ type: 'gaussian_blur', radius }),
   ...
   */
  brightnessContrast: (brightness: number, contrast: number): EffectConfig => ({
   ...
   */
  hueSaturation: (hue: number, saturation: number, lightness?: number): EffectConfig => ({
    type: 'hue_saturation', hue, saturation, lightness: lightness ?? 0,
  }),

  /** Invert all color channels. */
  invert: (): EffectConfig => ({ type: 'invert' }),
   ...
   */
  tint: (color: string, amount?: number): EffectConfig => ({
   ...
   */
  fill: (color: string, opacity?: number): EffectConfig => ({
