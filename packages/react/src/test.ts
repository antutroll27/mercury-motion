...
describe('@mmot/react', () => {
  ...
  test('addNull creates null layer', () => {
    ...
    scene.addNull('parent', {
      transform: {
        position: keyframes([
          { frame: 0, value: [50, 50] as [number, number] },
          { frame: 60, value: [80, 80] as [number, number] },
        ]),
      },
    })
  ...
  test('spring easing serializes correctly', () => {
    ...
    scene.addSolid('bouncer', {
      ...
      transform: {
        position: [
          { frame: 0, value: [100, 180] as [number, number], easing: { type: 'spring', mass: 1.0, stiffness: 170.0, damping: 26.0 } },
          { frame: 60, value: [100, 100] as [number, number] },
        ],
      },
    })
  ...
  test('interpolate with bezier easing', () => {
    ...
    const easing = kfs[0].easing as { type: string; x1: number; y1: number; x2: number; y2: number }
  ...
  test('spring creates keyframes with spring easing', () => {
    ...
    assert.strictEqual(kfs[0].frame, 0)
    assert.strictEqual(kfs[0].value, 0)
    const easing = kfs[0].easing as { type: string; stiffness: number; damping: number; mass: number }
  ...
  test('rect() creates rect shape', () => {
    ...
    if (s.shape_type !== 'rect') throw new Error('unreachable')
  ...
  test('ellipse() creates ellipse shape', () => {
    ...
    if (s.shape_type !== 'ellipse') throw new Error('unreachable')
  ...
  test('line() creates line shape', () => {
    ...
    if (s.shape_type !== 'line') throw new Error('unreachable')
  ...
  test('polygon() creates polygon shape', () => {
    ...
    if (s.shape_type !== 'polygon') throw new Error('unreachable')
  ...
  test('linearGradient() creates linear gradient config', () => {
    ...
    if (g.gradient_type !== 'linear') throw new Error('unreachable')
  ...
  test('radialGradient() creates radial gradient config', () => {
    ...
    if (g.gradient_type !== 'radial') throw new Error('unreachable')
  ...
  test('Effects.blur', () => {
    ...
    if (e.type !== 'gaussian_blur') throw new Error('unreachable')
  ...
  test('Effects.shadow with defaults', () => {
    ...
    if (e.type !== 'drop_shadow') throw new Error('unreachable')
  ...
  test('Effects.glow', () => {
    ...
    if (e.type !== 'glow') throw new Error('unreachable')
  ...
  test('Effects.brightnessContrast', () => {
    ...
    if (e.type !== 'brightness_contrast') throw new Error('unreachable')
  ...
  test('Effects.hueSaturation', () => {
    ...
    if (e.type !== 'hue_saturation') throw new Error('unreachable')
  ...
  test('Effects.tint', () => {
    ...
    if (e.type !== 'tint') throw new Error('unreachable')
  ...
  test('Effects.fill', () => {
    ...
    if (e.type !== 'fill') throw new Error('unreachable')
    assert.strictEqual(e.color, '#000000')
    assert.strictEqual(e.opacity, 0.5)
  })
  ...
  test('keyframes helper returns keyframes that serialize correctly', () => {
    ...
    scene.addSolid('s', {
      ...
      transform: {
        position: keyframes([
          { frame: 0, value: [0, 0] as [number, number], easing: 'ease_out' },
          { frame: 30, value: [100, 100] as [number, number] },
        ]),
      },
    })
  ...
  test('complete example scene with all features', () => {
    ...
    scene.addShape('star', {
      ...
      transform: {
        position: keyframes([
          { frame: 0, value: [200, 540] as [number, number], easing: 'ease_in_out' },
          { frame: 45, value: [880, 540] as [number, number] },
        ]),
        rotation: keyframes([
          { frame: 0, value: 0 },
          { frame: 45, value: 360 },
        ...
        scale: keyframes([
          { frame: 0, value: [0, 0] as [number, number], easing: Easing.easeOut },
          { frame: 8, value: [1, 1] as [number, number] },
          { frame: 37, value: [1, 1] as [number, number], easing: Easing.easeIn },
          { frame: 45, value: [0, 0] as [number, number] },
        ]),
      },
