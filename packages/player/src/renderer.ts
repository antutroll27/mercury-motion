...
export function renderFrame(ctx: CanvasRenderingContext2D, scene: any, frame: number, width: number, height: number): void {
  ...
  for (const layer of comp.layers) {
    ...
    if (frame < inPoint || frame >= outPoint) continue
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function renderLayer(ctx: CanvasRenderingContext2D, layer: any, frame: number, canvasWidth: number, canvasHeight: number): void {
  ctx.save()
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function drawSolid(ctx: CanvasRenderingContext2D, layer: any, canvasWidth: number, canvasHeight: number, isFillParent: boolean, pos: [number, number]): void {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function drawText(ctx: CanvasRenderingContext2D, layer: any): void {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function drawShape(ctx: CanvasRenderingContext2D, layer: any): void {
  ...
  switch (shape.shape_type) {
    ...
    case 'polygon': {
      const points = shape.points as [number, number][] | undefined
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function drawGradient(ctx: CanvasRenderingContext2D, layer: any, pos: [number, number], canvasWidth: number, canvasHeight: number, isFillParent: boolean): void {
  ...
  if (g.gradient_type === 'linear') {
    ...
    if (isFillParent) {
      ...
      grad = ctx.createLinearGradient(
        start[0] * canvasWidth - pos[0],
        start[1] * canvasHeight - pos[1],
        end[0] * canvasWidth - pos[0],
        end[1] * canvasHeight - pos[1],
      )
    }
  } else {
    // Radial gradient
    ...
    if (isFillParent) {
      grad = ctx.createRadialGradient(
        center[0] * canvasWidth,
        center[1] * canvasHeight,
        0,
        center[0] * canvasWidth,
        center[1] * canvasHeight,
        radius,
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function applyPreDrawEffects(ctx: CanvasRenderingContext2D, effects: any[] | undefined): void {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function applyMasks(ctx: CanvasRenderingContext2D, masks: any[]): void {
  for (const mask of masks) {
    ...
    switch (path.type) {
      ...
      case 'path': {
        const points = path.points as [number, number][] | undefined
