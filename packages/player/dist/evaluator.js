 ...
 */
function bezierComponent(p1, p2, t) {
    const c1 = 3 * p1;
    const c2 = 3 * (p2 - p1) - c1;
    const c3 = 1 - c1 - c2;
 ...
 */
function springSettlingTime(mass, stiffness, damping) {
 ...
 */
function spring(mass, stiffness, damping, t) {
    if (t <= 0)
        return 0;
    if (t >= 1)
        return 1;
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function evaluateValue(value, frame) {
    ...
    if (Array.isArray(value) && value.length > 0 && typeof value[0] === 'object' && 't' in value[0]) {
        ...
        let i = 0;
        while (i < kfs.length - 1 && kfs[i + 1].t <= frame)
            i++;
        const from = kfs[i];
        const to = kfs[i + 1];
        if (to.t === from.t)
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function evaluateVec2(value, frame) {
    if (value == null)
        return [0, 0];
    // Static [x, y]
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function evaluateColor(value, frame) {
